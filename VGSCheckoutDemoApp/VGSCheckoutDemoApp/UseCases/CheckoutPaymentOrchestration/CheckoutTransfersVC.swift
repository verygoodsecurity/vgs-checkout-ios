...
/*
 ...
 Step 3: Send request to payment orchestration or your own backend to create an order and get order_id.
 Step 4: Create `VGSCheckoutPaymentConfiguration` with access token and order_id.
 Step 5: Implement `VGSCheckoutDelegate` protocol to get notified when checkout transfer flow will be finished.
	...
	func checkoutButtonDidTap(in view: CheckoutFlowMainView) {
		...
		guard let newOrderId = orderId else {
			...
			paymentOrchestrationAPIClient.fetchToken {[weak self] payoptAccessToken in
				...
				options.methods = .savedCards(["FIN_ID_1", "FIN_ID_2"])

				strongSelf.paymentOrchestrationAPIClient.createOrder(with: 5300, currency: "USD", accessToken: payoptAccessToken) { orderId in
					VGSCheckoutPaymentConfiguration.createConfiguration(accessToken: payoptAccessToken, orderId: orderId, tenantId: DemoAppConfiguration.shared.paymentOrchestrationTenantId, environment: DemoAppConfiguration.shared.environment, options: options) { configuration in
...
extension CheckoutTransfersVC: VGSCheckoutDelegate {
	...
	func checkoutTransferDidFinish(with requestResult: VGSCheckoutRequestResult) {
		...
		case .failure(let statusCode, _, _, let error, _):
			title = "Checkout transfer flow status: Failed!"
