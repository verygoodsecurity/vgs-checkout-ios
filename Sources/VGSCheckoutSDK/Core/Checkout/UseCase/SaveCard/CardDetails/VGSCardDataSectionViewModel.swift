...
final internal class VGSCardDataSectionViewModel: VGSBaseFormSectionProtocol, VGSPlaceholderFieldViewDelegate {  
  ...
  /// Validation manager.
	...
	internal func buildForm() {
		...
		case .multiplexingAddCard(let multiplexingConfig):
			setupCardForm(with: multiplexingConfig)
		case .multiplexingPayment(let multiplexingConfig):
			setupCardForm(with: multiplexingConfig)
		}
	...
	private func setupCardForm(with multiplexingConfiguration: VGSCheckoutMultiplexingBasicConfiguration) {
