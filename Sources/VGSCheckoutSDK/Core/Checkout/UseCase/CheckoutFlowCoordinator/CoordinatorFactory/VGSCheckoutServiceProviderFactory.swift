...
internal class VGSCheckoutServiceProviderFactory {
	...
	static func buildCheckoutServiceProvider(with checkoutConfigurationType: VGSCheckoutConfigurationType, vgsCollect: VGSCollect, uiTheme: VGSCheckoutThemeProtocol) -> VGSCheckoutServiceProviderProtocol {
		...
		case .multiplexingAddCard(let configuration):
			return VGSCheckoutMultiplexingAddCardServiceProvider(multiplexingConfiguration: configuration, vgsCollect: vgsCollect, uiTheme: uiTheme)
		case .multiplexingPayment(let configuration):
			return VGSCheckoutMultiplexingPaymentServiceProvider(multiplexingConfiguration: configuration, vgsCollect: vgsCollect, uiTheme: uiTheme)
		}
	}
}
