...
internal class VGSCardDataFormConfigurationManager {
	...
	*/
	...
	internal static func setupCardForm(with configuration: VGSCheckoutPaymentOrchestrationBasicConfiguration, vgsCollect: VGSCollect, cardSectionView: VGSCardDetailsSectionView) {
    ...
    expDateConfiguration.inputSource = .keyboard
		...
		guard let cardHolderName = fieldViews.first(where: {$0.fieldType == .cardholderName}) else {
			assertionFailure("Invalid payopt setup! cardholder name not found!")
			return
		}
